...
<ul id="nav">
	...
	// get NCFR special pages: Directory and Browse by Topic
	$cpncfr_pages = get_posts( array( 
	
		'include' => cpncfr_get_special_pages(),
		'post_type' => 'page',
		'orderby' => 'post_date',
		'order' => 'DESC'
		
	) );
	//print_r( $cpncfr_pages ); die();
	
	// did we get them?
	if ( !empty( $cpncfr_pages ) ) {
		...
		foreach( $cpncfr_pages AS $post ) {
			...
			if ( $post->ID == $tmp_post->ID ) {
				$active_page = ' class="active_page"';
			}
	
			?>
			<li<?php echo $active_page; ?>><a href="<?php the_permalink() ?>" title="<?php the_title() ?>"><?php the_title() ?></a></li>
			<?php
